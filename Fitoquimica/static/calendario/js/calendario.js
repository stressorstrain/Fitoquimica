...
const full_year_today = date_today.getFullYear();
const all_months = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];
const days_31 = ['Janeiro', 'Março', 'Maio', 'Julho', 'Agosto', 'Outubro', 'Dezembro'];
const days_30 = ['Abril', 'Junho', 'Setembro', 'Novembro'];
const days_28 = ['Fevereiro'];
check_bissexto(full_year_today);
const month_number_today = date_today.getMonth();
const month_name_today = all_months[month_number_today];

var tables;

for(tables=0; tables <2; tables++)
        if(tables===0){
            let header = document.getElementById('month-year-navg');
            header.innerText= month_name_today + '  '+full_year_today;
            create_days(month_number_today, full_year_today, 'g');

        }
        else{
            let header = document.getElementById('month-year-navf');
            header.innerText= month_name_today + '  '+full_year_today;
            create_days(month_number_today, full_year_today, 'f');


        }

function prev0(tbs){
	let current = document.getElementById('month-year-nav'+tbs).innerText;
	...
	let table = document.getElementById('main_t'+tbs);
	let row;
	for(row=2; row<8; row++)
		table.rows[2].remove()
	create_days(current_prev_month_number, current_prev_year, tbs);

	document.getElementById('month-year-nav'+tbs).innerText =current_prev_month_name + ' ' + current_prev_year;
}
...
function next_month(tbs){
	...
	create_days(current_next_month_number, current_year, tbs);
	document.getElementById('month-year-nav'+tbs).innerText =current_next_month_name + ' ' + current_year;
}
...
function create_days(month, year, table_to){
	let week_n  = [0,1,2,3,4];
	let days_n = [0,1,2,3,4,5,6];
	let table = document.getElementById('main_t'+table_to);
	let tbody = document.getElementById('tbody'+table_to);
...
function insert_days(prev_month_lengths, current_first_day_number, table, current_month_lenghts, last_day){
	...
	while(x !== to_add){
		...
		tb_r.cells[x].style.color = '#CFCFCF';
		y = y-1;
		x++;
		}
...
function insert(first_row, first_null_cell, table){
	...
	for(it=first_row; it<8; it++){
		for(c_inx=first_null_cell; c_inx<7; c_inx++){
				...
				row[it].cells[c_inx].style.color = '#CFCFCF';
				count=count+1;
				first_null_cell = 0;
				nw_rw = true;
			}
